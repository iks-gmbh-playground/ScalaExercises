package com.iks.codewars.buyingACar
...
object BuyCar {
  ...
  def nbMonths(startPriceOld: Int,
    ...
    @tailrec
    def loop(currentPriceOld: Double, currentPriceNew: Double, currentPercent: Double, months: Int = 0): (Int, Double) = {
      val delta = currentPriceOld - currentPriceNew + months * savingPerMonth
      if (delta >= 0) (months, delta)
      else {
        val newPercent = if (months % 2 == 1) currentPercent + 0.5 else currentPercent
        val factor = 1 - newPercent / 100
        loop(currentPriceOld * factor, currentPriceNew * factor, newPercent, months + 1)
      }
    }
    val (months, delta) = loop(startPriceOld, startPriceNew, percentLossByMonth)
    Array(months, delta.round.toInt)
  }
