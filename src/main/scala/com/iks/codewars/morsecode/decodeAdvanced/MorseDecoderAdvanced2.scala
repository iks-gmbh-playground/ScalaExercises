...
which can be detected on a remote station. The Morse code encodes every character being transmitted as
a sequence of "dots" (short presses on the key) and "dashes" (long presses on the key).
...
1. Function decodeBits(bits), that should find out the transmission rate of the message, correctly decode
the message to dots ., dashes - and spaces (one between characters, three between words) and return those as
a string. Note that some extra 0's may naturally occur at the beginning and the end of a message,
...
*/
object MorseDecoderAdvanced2 {

  import com.iks.codewars.morsecode.decode.MorseCodes.morseCodes

  def decodeBits(bits: String): String =
    chunkAndNormalize(bits) collect {
      case "1" => "."
      case "111" => "-"
      case "000" => " "
      case "0000000" => "   "
  ...
  private def decodeWord(word: String) = (
    word
      split " "
      map morseCodes
      mkString ""
  ...
  private val chunkAndNormalize: String => List[String] =
    chunkBits _ andThen normalizeChunks

  private def normalizeChunks(chunks: List[String]): List[String] = {
    val widths = chunks.map(_.length())
    val unitWidth = widths.min

    chunks zip widths map {
      case (chunk, width) => chunk take (width / unitWidth)
    }
  }

  private def chunkBits(bits: String): List[String] =
    (trimBits(bits) foldRight(List[String](), "")) ((c, z) => z match {
      case (chunks, "") => (chunks, s"$c")
      case (chunks, s) => if (s.charAt(0) == c) (chunks, s"$c$s") else (s :: chunks, s"$c")
    }) match {
      case (chunks, "") => chunks
      case (chunks, s) => s :: chunks
    }

  private def trimBits(bits: String): String =
