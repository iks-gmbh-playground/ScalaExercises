...
/*
 * In this kata your task is to differentiate a mathematical expression
 * given as a string in prefix notation. The result should be the derivative
 * of the expression returned in prefix notation.
 *
 ...
 *
 * The operators and functions you are required to implement are + - * / ^ cos sin tan exp
 * ln where ^ means raised to power of. exp is the exponential function (same as e^x) and
 ...
 * (cos x) => (* -1 (sin x))
 *
 * In addition to returning the derivative your solution must also do some simplifications
 * of the result but only what is specified below:
 *
 * The returned expression should not have unnecessary 0 or 1 factors. For example it should
 * not return (* 1 (+ x 1)) but simply the term (+ x 1) similarly it should not return
 * (* 0 (+ x 1)) instead it should return just 0
 *
 * Results with two constant values such as for example (+ 2 2) should be evaluated and
 * returned as a single value 4
 *
 * Any argument raised to the zero power should return 1 and if raised to 1 should return the
 * same value or variable. For example (^ x 0) should return 1 and (^ x 1) should return x
 *
